  ...
  <div class="el-transfer-panel district-panel">
    ...
    <div class="el-transfer-panel__body">
      <div v-if="filterable" class="el-transfer-panel__filter el-input el-input--small el-input--prefix">
        <input type="text" v-model="searchWord" autocomplete="off" :placeholder="filterPlaceholder" class="el-input__inner" />
        <span class="el-input__prefix" style="left: 0px">
          <i class="el-input__icon el-icon-search"></i>
        </span>
      </div>
      <el-checkbox-group :class="{ expand: !filterable }" v-model="selectedDistrict" v-if="districtListMock.length > 0" @change="handleCheckedChange">
        <el-checkbox v-for="(city, index) in districtListMock" class="el-transfer-panel__item" :disabled="city.disabled" :title="city.label" :label="city" :key="index">{{ city.label }}</el-checkbox>
      </el-checkbox-group>
      <p class="no-data" v-else>无数据</p>
    </div>
    <div class="vip-footer">
      <el-button type="text" :disabled="selectedDistrict.length > 0 ? false : true" size="small" round @click="checkedSelected">
        <span>{{ operation }}</span>
      </el-button>
    </div>
  </div>
