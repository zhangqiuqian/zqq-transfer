<template>
  <div class="zqq-main">
    <zqq-box ref="noSelect" :title="boxTitle[0]" :operateId="0" :dataShowList="notSelectDataList" :pageSize="pageSize" :filterable="filterable" :filter-placeholder="filterPlaceholder[0]" :pageTexts="pageTexts" :async="async" :isLastPage="isLastPage" :isHighlight="isHighlight" :highlightColor="highlightColor" :asyncSearchFlag="asyncSearchFlag" :showClearBtn="showClearBtn" @check-district="noCheckSelect" @search-word="searchWord" @check-disable="checkDisable" @get-data="getData" @get-data-by-keyword="getDataByKeyword" @clear-input="clearQueryInp('left')"></zqq-box>
    <div class="opera">
      <el-button class="el-transfer__button" icon="el-icon-arrow-left" type="primary" circle @click="deleteData" :disabled="disablePre"></el-button>
      <el-button class="el-transfer__button" icon="el-icon-arrow-right" type="primary" circle @click="addData" :disabled="disableNex"></el-button>
    </div>
    <zqq-box ref="hasSelect" :title="boxTitle[1]" :operateId="1" :dataShowList="checkedData" :pageSize="pageSize" :filterable="filterable" :filter-placeholder="filterPlaceholder[1]" :pageTexts="pageTexts" :isHighlight="isHighlight" :highlightColor="highlightColor" :showClearBtn="showClearBtn" @check-district="hasCheckSelect" @search-word="searchWord" @check-disable="checkDisable" @clear-input="clearQueryInp('right')"></zqq-box>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    watch: {
      selectIdList(newVal) {
        // 获取已选数据的监听事件
        const moveKeys = [...this.noCheckData.map(item => item.id), ...this.hasCheckData.map(item => item.id)]
        this.hasCheckData = []
        this.noCheckData = []
    ...
    methods: {
      ...
      initData(selectedChange) {
        ...
        if (!this.async) {
          ...
          const checkDataId = this.selectListCheck.map(ele => ele.id)
          this.notSelectDataList = this.originList.filter(ele => !checkDataId.includes(ele.id) && (ele.label.includes(this.noSelectkeyword) || this.asyncSearchFlag))
      ...
      addData() {
        ...
        // 待选区数据过滤
        // 如果设置了异步搜索，就不用过滤关键词 this.asyncSearchFlag 为 true
        this.notSelectDataList = this.notSelectDataList.filter(ele => !noCheckDataId.includes(ele.id) && (ele.label.includes(this.noSelectkeyword) || this.asyncSearchFlag))
        ...
        if (!this.async && this.sort) {
          ...
          this.checkedData = this.originList.filter(ele => !dataListNoCheckId.includes(ele.id) && ele.label.includes(this.haSelectkeyword))
      ...
      deleteData() {
        ...
        this.checkedData = this.checkedData.filter(ele => !hasCheckDataId.includes(ele.id) && ele.label.includes(this.haSelectkeyword))
        ...
        // 如果设置了异步搜索，就不用过滤关键词 this.asyncSearchFlag 为 true
        this.notSelectDataList = this.originList.filter(ele => !selectListCheckId.includes(ele.id) && (ele.label.includes(this.noSelectkeyword) || this.asyncSearchFlag))
