  ...
  <div class="el-transfer-panel district-panel">
    ...
    <div class="el-transfer-panel__body">
      <div v-if="filterable" class="el-transfer-panel__filter el-input el-input--small el-input--prefix">
        <input type="text" v-model.trim="searchWord" autocomplete="off" @change="handleKeyword" :placeholder="filterPlaceholder" class="el-input__inner" :class="{ showClear: showClearBtn }" />
    ...
    methods: {
      ...
      pageData() {
        ...
        if (this.total > 1 && this.pageIndex < this.total - 1) {
          ...
          this.districtListMock = this.dataShowList.slice(this.pageIndex * this.pageSize, this.pageIndex * this.pageSize + this.pageSize)
        } else {
      ...
      asyncInitData() {
        ...
        this.disabledPre = this.asyncSearchFlag && this.asyncSearch ? this.asyncSearchPageIndex <= 1 : this.asyncPageIndex <= 1
        // 赋值
      ...
      prev() {
        if (this.async) {
          ...
          this.asyncSearchFlag && this.asyncSearch ? this.$emit('get-data-by-keyword', this.searchWord, this.asyncSearchPageIndex <= 1 ? 1 : --this.asyncSearchPageIndex) : this.$emit('get-data', this.asyncPageIndex <= 1 ? 1 : --this.asyncPageIndex)
        } else {
      ...
      next() {
        if (this.async) {
          // 异步获取数据
          this.disabledNex = true
          this.asyncSearchFlag && this.asyncSearch ? this.$emit('get-data-by-keyword', this.searchWord, ++this.asyncSearchPageIndex) : this.$emit('get-data', ++this.asyncPageIndex)
        } else {
